<template>
    <gmap-map id="google-map" :center="center" ref="mapRef" :zoom="zoom" :options="mapStyle">
        ...
        <gmap-info-window @closeclick="window_open=false"
                          :opened="window_open"
                          :position="markerPos"
                          :options="{
              pixelOffset: {
            ...
            <h2>
                ...
                <v-chip v-if="markerItem.agencyType != '-1'"
                        class="ma-2"
                        color="primary"
                        outlined>{{ markerItem.agencyType }}</v-chip>
            </h2>
            <v-container class>
                ...
                <v-dialog v-model="sheet" height="90%" width="500px">
                    ...
                    <v-card scrollable>
                        <v-card style="border-radius:0;" class="mx-auto mb-5" color="#26c6da" dark>
                            ...
                            <v-card-text class="headline py-1 font-weight-regular">Start:{{routeInfo.startaddress}}</v-card-text>
                            <v-card-text class="headline py-1 font-weight-regular">End: {{routeInfo.endaddress}}</v-card-text>
                            <v-card-text class="headline py-1 font-weight-regular">Total Distance:{{routeInfo.distance}}</v-card-text>
                            <v-card-text class="headline py-1 mb-5 font-weight-regular">Total Duration:{{routeInfo.duration}}</v-card-text>
                        </v-card>
