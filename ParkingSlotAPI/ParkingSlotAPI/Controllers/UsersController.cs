...
namespace ParkingSlotAPI.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        [AllowAnonymous]
        [HttpPost("CheckToken")]
        public IActionResult CheckToken([FromBody] UserForUpdatePasswordDto userForUpdatePasswordDto)
        {
            try
            {
        ...
        public IActionResult ConfirmPassword([FromBody] UserForUpdatePasswordDto userForUpdatePasswordDto)
        {
            ...
            try
            {
                ...
                if (!_userRepository.UserExists(userForUpdatePasswordDto.Id))
                {
                    throw new AppException("The user does not exist in the database.");
                }

                var userFromRepo = _mapper.Map<User>(userForUpdatePasswordDto);

                userFromRepo = _userRepository.GetUser(userFromRepo.Id);

                _userRepository.UpdatePassword(userFromRepo, userForUpdatePasswordDto.NewPassword);
